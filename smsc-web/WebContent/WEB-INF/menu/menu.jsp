						...
						<c:choose>
							<c:when test="${not empty submenu.subElementSubIndex}">
								<c:set var="menulink" scope="session"
									value="&subid=${submenu.subElementSubIndex}" />
							</c:when>
							<c:otherwise>
								<c:set var="menulink" scope="session"
									value="${submenu.subElementSubIndex}" />
							</c:otherwise>
						</c:choose>

						<c:if test="${submenu.mainMenuId == menu.mainMenuId}">
							<li ${menu.elementClass}><a
								href="${pageContext.servletContext.contextPath}/${menu.pageStr}?id=${menu.pageIndex}${menulink}"><span>${submenu.title}</span></a>
						</c:if>
					</c:forEach>
				</ul>
		</c:forEach>
